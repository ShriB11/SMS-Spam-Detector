...
def preprocess(df):
    ...
    clean = [[word for word in lines if word != ',' if word != '.']
             for lines in df['tokenized']]
    df.insert(2, "clean", clean)

    # Stem the words
    porter_stemmer = PorterStemmer()
    # Stem the words
    stemmed_words = [[porter_stemmer.stem(
        word=word) for word in lines] for lines in df['tokenized']]
    ...
    lemmatized = [[lemmatizer.lemmatize(
        word=word, pos='v') for word in lines] for lines in df['stemmed']]
    df.insert(4, "lemmatized", lemmatized)

    # stop words removal
    stop_words = set(stopwords.words('english'))
    stop_remove = [[lemmatizer.lemmatize(
        word=word, pos='v') for word in lines if word not in stop_words] for lines in df['stemmed']]
